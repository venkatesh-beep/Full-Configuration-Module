...
def paycode_event_sets_ui():
    ...
    if st.button("⬇️ Download Template", use_container_width=True):
        ...
        events_df = (
            pd.DataFrame([
                ...
                for e in r.json()
            ]) if r.status_code == 200 else pd.DataFrame(columns=["id", "name", "description"])
        )
    ...
    # ==================================================
    st.markdown("### 📤 Upload Paycode Event Sets")
                ...
                for row_no, row in df.iterrows():
                    try:
                        ...
                        for i in range(1, 6):
                            ...
                            priority = int(priority_raw) if str(priority_raw).isdigit() else i
    ...
    # ==================================================
    st.markdown("### 🗑️ Delete Paycode Event Sets")
    ...
    # ==================================================
    st.markdown("### ⬇️ Download Existing Paycode Event Sets")
        ...
        with st.spinner("⏳ Fetching Paycode Event Sets..."):
            ...
            if r.status_code != 200:
                ...
                for s in r.json():
                    ...
                    for idx, entry in enumerate(sorted(s.get("entries", []), key=lambda x: x.get("priority", 0)), start=1):
                ...
                events_df = (
                    pd.DataFrame([
                        ...
                        for e in r2.json()
                    ]) if r2.status_code == 200 else pd.DataFrame(columns=["id", "name", "description"])

                output = io.BytesIO()
                with pd.ExcelWriter(output, engine="openpyxl") as writer:
                    df.to_excel(writer, index=False, sheet_name="Paycode_Event_Sets")
                    events_df.to_excel(writer, index=False, sheet_name="Available_Paycode_Events")
