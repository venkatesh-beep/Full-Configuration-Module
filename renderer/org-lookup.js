/* =========================================================
 * org-lookup.js — BeeForce Configuration Portal
 ...
 * ========================================================= */
// =========================================================
// modules/organization-lookup.js — BeeForce Configuration Portal
// Owns: screen-ol
// Endpoints:
//   GET  /api/attendance/organization_location_lookup_table
//   POST /api/attendance/organization_location_lookup_table/action  {action:"SAVE", table:{...}}
...
function init(ctx) {
  ...
  const prefix = "ol";
  const entityType = "ORGANIZATION_LOCATION";
  const getUrl = () => `${AppState.BASE_URL}/api/attendance/organization_location_lookup_table`;
  const postUrl = () => `${AppState.BASE_URL}/api/attendance/organization_location_lookup_table/action`;

  if (!document.getElementById("screen-ol")) {
    const main = document.createElement("div");
    main.id = "screen-ol"; main.style.display = "none";
    main.innerHTML = `
      <div class="bf-breadcrumb"><span class="bc-link" data-go="screen-hub">🏠 Home</span><span class="bc-sep">›</span><span class="bc-cur">🏢 Org Location Lookup</span></div>
      <div class="bf-module-header"><div class="mh-icon">🏢</div><div><div class="mh-title">Organization Location Lookup</div><div class="mh-sub">Download · Upload</div></div></div>
      <div id="ol-status" class="bf-status"></div>
      <button class="bf-btn bf-primary" id="ol-download">⬇️ Download the Existing Data</button>
      <div class="bf-divider"></div>
      <div class="bf-info-box">Red highlighted columns are <b>INPUT (mandatory)</b> fields.</div>
      <label class="bf-file-label" for="ol-file"><span id="ol-file-name">📂 Click to select filled file (.xlsx)</span></label>
      <input type="file" id="ol-file" class="bf-file-input" accept=".xlsx,.xls,.csv" />
      <div class="bf-progress" id="ol-prog"><div class="bf-pb" id="ol-bar" style="width:0%"></div></div>
      <div class="bf-log" id="ol-log"></div>
      <button class="bf-btn bf-success" id="ol-submit" disabled>🚀 Validate & Upload</button>`;
    modal.appendChild(main);

    Router.register("screen-ol");
  ...
  async function fetchLookupTable(url) {
    const r = await ApiTracker.fetch(url, { headers: apiH() }, "Org Lookup");
  ...
  document.getElementById(`${prefix}-download`).onclick = async () => {
    ...
    try {
      ...
      downloadExcel(`organization_location_lookup_data.xlsx`, [{ name: "Existing_Data", tabColor: "1D4ED8", headers: columns, rows, highlightCols: inputColIdx }]);
      ...
      Notifications.doneLoading(btn, "⬇️ Download the Existing Data");
    }
  };
  ...
  document.getElementById(`${prefix}-submit`).onclick = async () => {
    ...
    if (validationErrors.length) {
      ...
      validationErrors.forEach(e => {
        ...
        Audit.onValidationError(`Org Lookup row ${e.Row} "${e.Field}": ${e.Error}`);
      });
